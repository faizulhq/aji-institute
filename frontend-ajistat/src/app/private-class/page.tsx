...
import { PRIVATE_PROGRAMS } from '@/lib/config';
    ...
    <Link href={`/program/${p.slug}`}
        ...
        {p.image ? (
          ...
          <>
            ...
            <div className="relative text-center px-4">
              <p className="text-white/60 text-xs font-semibold uppercase tracking-widest">Kelas Privat</p>
      ...
      <div className="p-5 flex flex-col flex-1">
        ...
        <div className="mt-auto">
          ...
          <span className="block w-full text-center bg-[#162058] group-hover:bg-[#2348A8] text-white font-bold py-2.5 rounded-xl text-sm transition-colors mt-3">
            Lihat Detail
          </span>
        </div>
      </div>
...
export default async function PrivateClassPage() {
  let programs: ApiProgram[] = await fetchPrograms({ type: 'private-class' });

  if (programs.length === 0) {
    programs = PRIVATE_PROGRAMS.map((p) => ({
      id: p.id, title: p.title, slug: p.slug, type: 'private-class' as const,
      status: 'upcoming' as const, price: p.price, original_price: p.originalPrice,
      tags: p.tags, duration: p.duration, schedule: p.status ?? 'Tersedia',
      facilitator_name: p.facilitator, facilitator_bio: p.facilitatorBio,
      thumbnail_color: '#1B3A8C', is_featured: false,
      description: p.description, curriculum: p.curriculum,
    }));
  }
    ...
    <>
      <div className="bg-[#162058] relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <h1 className="text-4xl sm:text-5xl font-black text-white mb-4 leading-tight">Kelas Privat</h1>
          <p className="text-white/70 max-w-2xl text-lg leading-relaxed">
            Pembelajaran 1-on-1 dengan jadwal fleksibel. Materi disesuaikan 100% dengan kebutuhan dan dataset riset Anda.
          </p>
        </div>
      </div>
