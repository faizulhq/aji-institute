...
import { SHORT_CLASS_PROGRAMS } from '@/lib/config';
...
export default async function ShortClassPage() {
  let programs: ApiProgram[] = await fetchPrograms({ type: 'short-class' });

  if (programs.length === 0) {
    programs = SHORT_CLASS_PROGRAMS.map((p) => ({
      id: p.id, title: p.title, slug: p.slug, type: 'short-class' as const,
      status: 'upcoming' as const, price: p.price, original_price: p.originalPrice,
      tags: p.tags, duration: p.duration, schedule: p.status ?? 'Akan Datang',
      facilitator_name: p.facilitator, facilitator_bio: p.facilitatorBio,
      thumbnail_color: '#162058', is_featured: false,
    ...
    <>
      <div className="bg-[#162058] relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
          ...
          <h1 className="text-4xl sm:text-5xl font-black text-white mb-4 leading-tight">Short Class</h1>
          <p className="text-white/70 max-w-2xl text-lg leading-relaxed">
            Kelas singkat 2–3 jam, fokus pada satu topik statistik. Terjangkau, praktis, dan langsung bisa diterapkan.
          </p>
        </div>
      </div>
