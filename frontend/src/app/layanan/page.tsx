    ...
    <>
        ...
        <div className="max-w-4xl mx-auto px-4 text-center">
          ...
          <div className="flex flex-wrap justify-center gap-4 mt-8">
            <a href={WA_LINK('Halo, saya ingin konsultasi layanan analisis data')}
              ...
              className="inline-flex items-center gap-2 bg-[#F0A500] hover:bg-[#C8870A] text-[#0C1A45] font-bold px-7 py-3.5 rounded-xl transition-colors">
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <p className="text-[#2568B5] text-sm font-semibold uppercase tracking-widest mb-3">Apa yang Kami Kerjakan</p>
      ...
      <section className="py-20 bg-white">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
            {SOFTWARE.map((sw) => (
              <div key={sw.name} className="bg-gray-50 border border-gray-100 rounded-xl p-4 text-center hover:border-[#2568B5]/30 hover:bg-blue-50 transition-colors">
      ...
      <section className="py-20 bg-[#0C1A45]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <p className="text-[#4FA8D8] text-sm font-semibold uppercase tracking-widest mb-3">Bagaimana Proses Kami</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {PROSES.map((p) => (
              <div key={p.step} className="bg-white/8 border border-white/15 rounded-2xl p-6 flex gap-4">
                <div className="w-12 h-12 bg-[#2568B5] rounded-xl flex items-center justify-center text-white font-bold text-lg shrink-0">
                  {p.step}
                </div>
                <div>
      ...
      <section className="py-20 bg-gray-50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-10">
            <p className="text-[#2568B5] text-sm font-semibold uppercase tracking-widest mb-3">Estimasi Biaya</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              { tier: 'Menengah', range: 'Rp 350.000 – 750.000', items: ['SEM / Path Analysis', 'Analisis multivariat', 'Uji mediasi/moderasi', 'Laporan lengkap', 'Revisi 1x'], popular: true, color: 'border-[#2568B5]' },
              ...
              <div key={pkg.tier} className={`bg-white rounded-2xl border-2 ${pkg.color} p-6 shadow-sm relative`}>
                {pkg.popular && (
                  <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-[#F0A500] text-[#0C1A45] text-xs font-bold px-3 py-1 rounded-full">
                    ⭐ Paling Diminati
                  </div>
                )}
                <h3 className="font-bold text-gray-900 text-lg mb-1">{pkg.tier}</h3>
                <p className="text-[#2568B5] font-bold text-xl mb-4">{pkg.range}</p>
                ...
                <a href={WA_LINK(`Halo, saya ingin tanya paket konsultasi ${pkg.tier}`)}
                  target="_blank" rel="noopener noreferrer"
                  className="mt-6 block text-center py-2.5 rounded-xl text-sm font-semibold bg-[#162660] text-white hover:bg-[#2568B5] transition-colors">
                  Tanya Sekarang
                </a>
              </div>
            ))}
      ...
      {/* CTA */}
      <section className="py-14 bg-[#162660]">
